...
def main():
    ...
    emission = np.exp(-((x - 600) ** 2) / (2 * sigma**2)) / (
    ...
    coherent_ld = camera.get_image(
        coherent, shot_noise=False, amplifier_noise=False
    )
    ...
    poly_ld = camera.get_image(
        poly, shot_noise=args.noise, amplifier_noise=args.noise
    )
