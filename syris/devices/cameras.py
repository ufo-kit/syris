...
class Camera(object):
    ...
    def __init__(self, pixel_size, gain, dark_current, amplifier_sigma, bits_per_pixel,
                 shape, quantum_efficiencies=None, wavelengths=None, exp_time=None, fps=None,
                 dtype=np.ushort):
        ...
        *dark_current* as mean number of electrons present without incident light, *amplifier_sigma*
        is the sigma of the normal distributed noise given by camera electronics, *bit_per_pixel* is
        the number of bits holding the pixel grey value, *quantum_efficiencies* are the quantum
        efficiencies at *wavelengths*, *exp_time* is the exposure time and *fps* are Frames Per
        Second which are generated by the camera (exposure time is independent from fps, i.e.  e.g.
        fps can be set to 1000 and exposure time to 1 \mu s, but it cannot exceed :math:`1/fps` s).
        *dtype* is the sensor output data type.
        """
