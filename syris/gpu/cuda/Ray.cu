...
{
    return __frsqrt_rn(v.x * v.x + v.y * v.y + v.z * v.z);
}
...
*/
// __device__ bool Ray::intersects (FP_T4 const &minBbox, FP_T4 const &maxBbox, FP_T &tmin, FP_T &tmax) {
//     FP_T4 t_lower = (minBbox - this->tail) * this->invDirection;
//     FP_T4 t_upper = (maxBbox - this->tail) * this->invDirection;
...
{
    constexpr FP_T EPSILON = 1e-6;
    FP_T tmin = 0, tmax = INFINITY;
...
__device__ bool Ray::intersects(FP_T4 const &V1, FP_T4 const &V2, FP_T4 const &V3, FP_T &tmin, FP_T &tmax) const {
    ...
    if (is_null(U) || is_null(V) || is_null(W)) {
...
//     FP_T4 const P = cross(dir, e_2);

//     constexpr FP_T EPSILON = ::cuda::std::numeric_limits<double>::epsilon();
//     // constexpr FP_T EPSILON = 1e-6;
